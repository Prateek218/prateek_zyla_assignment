...
const url='http://localhost:3000/getData?task=task3';
Http.open("GET", url);
var selectedArray = [];
var vowels = /[aeiou]/i;
...
contentSplits.map((tokenMapping, idt) => {
  ...
  tokenWrapping.addEventListener("click", () => {
    var isBaky = vowels.test(token.charAt(0))
    var available = false;
    ...
    if(!available){
      ...
      if(isBaky){
        tokenWrapping.title = "Baky";
        selectedItem.classList.add("selectedv");
      }else{
        tokenWrapping.title = "Kola";
        selectedItem.classList.add("selected");
      }
    }else{
      selectedItem = tokenWrapping;
      tokenWrapping.removeAttribute('title');
  ...
  for(var i in x){
    ...
    if (storedPos === position) {
        ...
        if(isBaky){
          tokenWrapping.title = "Baky";
          tokenWrapping.classList.add("selectedv");
        }else{
          tokenWrapping.title = "Kola";
          tokenWrapping.classList.add("selected");
        }
        tokenWrapping.classList.getSelection
...
function sendData(selectedArray){
  ...
  xhttp.send("position="+selectedArray+"&task=task3");
}
